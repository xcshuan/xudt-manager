...
use ckb_sdk::{Address, AddressPayload, CkbRpcClient, NetworkInfo, SECP256K1};
use ckb_sdk::constants::SIGHASH_TYPE_HASH;
use ckb_sdk::traits::{CellCollector, CellQueryOptions, DefaultCellCollector, ValueRangeOption};
...
use ckb_types::packed::{Bytes as PackBytes, CellDep, CellInput, CellOutput, OutPoint};
use ckb_types::core::{DepType, ScriptHashType};
use ckb_types::{h256, H256};
use ckb_types::packed::Script;
use ckb_types::prelude::{Builder, Entity, Pack};
use xudt_manager::handler::XudtHandler;
use xudt_manager::XudtTransactionBuilder;

const UNIQUE_ARGS_SIZE: usize = 20;
const ISSUE_DECIMAL: u8 = 8;
const ISSUE_NAME: &'static str = "XUDT Test B Token";
...
fn main() -> Result<(), Box<dyn StdErr>>{
    ...
    configuration.register_script_handler(Box::new(XudtHandler::new_with_network(&network_info)?) as Box<_>);
    ...
    builder.add_output_and_data(
        CellOutput::new_builder()
            ...
            .capacity((calculate_udt_cell_capacity(issue_lock_script.clone()) as u64 * NANOS_PER_SEC).pack())
            .build(),
    ...
    builder.add_output_and_data(
        CellOutput::new_builder()
            ...
            .capacity((calculate_xudt_token_info_cell_capacity(issue_lock_script.clone()) as u64 * NANOS_PER_SEC).pack())
            .build(),
...
fn encode_token_info() -> Vec<u8> {
    ...
    [decimal.as_bytes(), name_size.as_bytes(), name.as_bytes(), symbol_size.as_bytes(), symbol.as_bytes()].concat()
}
